...
function renderContent(content: string) {
  ...
  const flushList = () => {
    if (listItems.length > 0) {
      elements.push(
        <ul key={`list-${elements.length}`} className="list-disc list-inside space-y-1 text-slate-300 text-sm mb-3">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700">
                {headers.map((h, i) => (
                  <th key={i} className="text-left py-1.5 px-2 text-cyan-400 font-medium">{renderInline(h)}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {body.map((row, ri) => (
                <tr key={ri} className="border-b border-slate-800">
                  {row.map((cell, ci) => (
                    <td key={ci} className="py-1.5 px-2 text-slate-300">{renderInline(cell)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
  ...
  const flushCode = () => {
    if (codeLines.length > 0) {
      elements.push(
        <pre key={`code-${elements.length}`} className="bg-slate-800 border border-slate-700 rounded-lg px-4 py-3 text-sm text-cyan-300 font-mono overflow-x-auto mb-3">
          {codeLines.join('\n')}
        </pre>
      )
      codeLines = []
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.trim().startsWith('`Tip:`')) {
      ...
      elements.push(
        <div key={`tip-${elements.length}`} className="bg-cyan-900/20 border border-cyan-800/40 rounded-lg px-4 py-2.5 text-sm text-cyan-300 mb-3 flex items-start gap-2">
    ...
    if (line.trim().startsWith('**') && line.trim().endsWith('**')) {
      ...
      elements.push(
        <h4 key={`h4-${elements.length}`} className="text-sm font-semibold text-slate-200 mt-4 mb-2">{text}</h4>
      )
      continue
    }
    ...
    // Regular paragraphs
    elements.push(
      <p key={`p-${elements.length}`} className="text-sm text-slate-300 leading-relaxed mb-3">{renderInline(line)}</p>
    )
  }
...
function renderInline(text: string): React.ReactNode {
  ...
  while (remaining.length > 0) {
    ...
    if (earliest.type === 'bold') {
      parts.push(
        <strong key={`b-${keyIdx++}`} className="text-slate-200 font-semibold">{earliest.match[1]}</strong>
      )
    } else if (earliest.type === 'code') {
      parts.push(
        <code key={`c-${keyIdx++}`} className="bg-slate-700 rounded px-1.5 py-0.5 font-mono text-xs text-cyan-300">{earliest.match[1]}</code>
      )
    }
...
export function HelpPage() {
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(helpSections.map((s) => s.id)))
                    ...
                    {section.subsections.map((sub) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setTimeout(() => {
                            document.getElementById(`help-${sub.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
                          }, 50)
                        }}
                  ...
                  {searchResults.map(({ section, subsection }) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        handleSectionClick(section.id)
                        setTimeout(() => {
                          document.getElementById(`help-${subsection.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
                        }, 50)
                      }}
