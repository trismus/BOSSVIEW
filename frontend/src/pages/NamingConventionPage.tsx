...
function validateName(name: string): { valid: boolean; errors: string[]; parsed?: ParsedValid } {
  ...
  if (result.category === 'workstation' || result.category === 'server') {
    ...
    if (dt && result.sequence.length !== dt.seqLength) {
      errors.push(
        `Sequenz sollte ${dt.seqLength}-stellig sein, ist aber ${result.sequence.length}-stellig.`,
      );
    }
  }
...
function resolveVlan(vlanId: number): VlanResolution | null {
  for (const [locCode, block] of Object.entries(VLAN_BLOCKS)) {
    if (vlanId >= block.start && vlanId <= block.end) {
      ...
      const funcOffsets = Object.keys(VLAN_FUNCTIONS)
        .map(Number)
        .sort((a, b) => a - b);
            ...
            {parseResult.valid && parsedDetails.valid ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                {[
                  ...
                  {
                    label: 'Location',
                    value: `${parsedDetails.locationCode} (${parsedDetails.resolvedName})`,
                  },
              ...
              <div className="bg-red-900/20 border border-red-800/50 rounded-lg p-3">
                <p className="text-sm text-red-400">{parseResult.errors.join(' ')}</p>
              </div>
            )}
          </div>
        )}
      ...
      >
        ...
        <div className="mb-8">
          ...
          <div className="mt-3 flex items-start gap-2 bg-amber-900/20 border border-amber-800/40 rounded-lg px-4 py-2.5">
            ...
            <p className="text-sm text-amber-300/90">
              <code className="font-mono text-amber-400">lidozrh</code> devices are physically
              located at Stelzenstrasse (STS)
            </p>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-left py-2.5 px-3 text-slate-400 font-medium">
                    Ex: Management
                  </th>
                  <th className="text-left py-2.5 px-3 text-slate-400 font-medium">
                    Ex: Production
        ...
        <div>
          ...
          <p className="text-sm text-slate-400 mb-3 font-mono">
            10.<span className="text-amber-400">{'{Location}'}</span>.
            <span className="text-cyan-400">{'{VLAN}'}</span>.
            <span className="text-purple-400">{'{Host}'}</span>/24
          </p>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
            ...
            {vlanResult ? (
              ...
              <div className="bg-red-900/20 border border-red-800/50 rounded-lg p-3 max-w-2xl">
                <p className="text-sm text-red-400">
                  VLAN {vlanInput} does not match any known location block (STS: 10–99, BAS:
                  100–199, PCT: 200–299).
                </p>
              </div>
            )}
