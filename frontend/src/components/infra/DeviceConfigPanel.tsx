...
export function DeviceConfigPanel({
  ...
  const tabs: { key: Tab; label: string; disabled?: boolean }[] = [
    ...
    {
      key: 'interfaces',
      label: `Interfaces${parsed ? ` (${parsed.interfaces.length})` : ''}`,
      disabled: !parsed,
    },
    {
      key: 'port-channels',
      label: `Port-Channels${parsed ? ` (${parsed.portChannels.length})` : ''}`,
      disabled: !parsed,
    },
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
          ...
          {tab === 'port-channels' && parsed && (
            <PortChannelTable portChannels={parsed.portChannels} />
          )}
          {tab === 'history' && (
            <HistorySection history={history} loading={loadingHistory} onView={viewConfig} />
          )}
        </div>
      </div>
    </div>
      ...
      {parsed && (
        <div style={{ marginTop: 24 }}>
          ...
          >
            ...
            <SummaryCard
              label="Interfaces"
              value={String(parsed.interfaces.length)}
              color={C.green}
            />
            <SummaryCard
              label="Port-Channels"
              value={String(parsed.portChannels.length)}
              color={C.amber}
            />
        ...
        <tbody>
          {sorted.map((v) => (
            <tr key={v.id} style={{ borderBottom: `1px solid ${C.border}` }}>
              <td style={{ padding: '8px 12px', color: C.cyan, fontFamily: 'monospace' }}>
                {v.id}
              </td>
              ...
              <td style={{ padding: '8px 12px' }}>
                <StatusBadge
                  status={v.state}
                  colorMap={{ active: C.green, suspend: C.amber, unknown: C.textMuted }}
                />
              </td>
            </tr>
          ))}
...
function HistorySection({
  ...
  if (loading) {
    return (
      <div style={{ color: C.textDim, textAlign: 'center', padding: 32 }}>Loading history...</div>
    );
  }
        ...
        <tbody>
          {history.map((entry) => (
            <tr key={entry.id} style={{ borderBottom: `1px solid ${C.border}` }}>
              ...
              <td style={{ padding: '8px 12px', color: C.cyan, textAlign: 'center' }}>
                {entry.vlan_count}
              </td>
              <td style={{ padding: '8px 12px', color: C.green, textAlign: 'center' }}>
                {entry.interface_count}
              </td>
              <td style={{ padding: '8px 12px' }}>
                <button
