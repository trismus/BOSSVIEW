...
import {
  PORT_STATUS_COLORS as PORT_COLORS,
  INFRA_COLORS_MINIMAL as C,
  UI_CHROME,
} from '../../styles/data-viz-colors'
...
export function PortGrid({ interfaces, deviceName, onPortClick }: PortGridProps) {
  ...
  const gridWidth = maxCols * (portW + gap) + (uplinkPorts.length > 0 ? uplinkPorts.length * (uplinkW + gap) + 20 : 0) + 40
  ...
  return (
    <div style={{ background: C.bg, borderRadius: 8, padding: 16, border: `1px solid ${C.border}` }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div>
          <span style={{ fontSize: 12, color: C.cyan, fontFamily: 'JetBrains Mono, monospace', fontWeight: 600 }}>
            {deviceName}
          </span>
        ...
        >
          ...
          {topRow.map((iface, i) => {
            ...
            return (
              ...
              >
                <rect
                  x={x} y={y} width={portW} height={portH}
                  rx={2} fill={color}
                ...
                {/* Port number label */}
                <text
                  x={x + portW / 2} y={y + portH + labelH - 2}
            ...
            <>
              ...
              {uplinkPorts.map((iface, i) => {
                ...
                return (
                  ...
                  >
                    <rect
                      x={x} y={y} width={uplinkW} height={uplinkH}
                      rx={3} fill={color}
                      stroke={isSelected ? '#fff' : 'transparent'}
                      strokeWidth={isSelected ? 1.5 : 0}
                      opacity={iface.status === 'admin-down' ? 0.5 : 0.9}
                    />
                    <text
                      x={x + uplinkW / 2} y={y + uplinkH + labelH - 2}
                      textAnchor="middle" fill={C.textMuted}
                      fontSize={6} fontFamily="JetBrains Mono, monospace"
                    >
                      {extractPortNumber(iface.name)}
        ...
        {[
          ...
          <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{
              ...
              background: item.color, opacity: item.color === PORT_COLORS.adminDown ? 0.5 : 0.9,
            }} />
      ...
      {hoveredPort && (
        <div style={{
          ...
          <div style={{ fontSize: 10, color: C.textDim }}>
            Status: <span style={{ color: hoveredPort.status === 'up' ? PORT_COLORS.upAccess : PORT_COLORS.down }}>
              {hoveredPort.status}
            </span>
          </div>
          <div style={{ fontSize: 10, color: C.textDim }}>
      ...
      {selectedPort && (
        <div style={{
          ...
          background: UI_CHROME.bgOverlay, border: `1px solid ${C.border}`,
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: C.cyan, fontWeight: 600, fontFamily: 'JetBrains Mono, monospace' }}>
              {selectedPort.name}
            </span>
            <button
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 8, fontSize: 11 }}>
            ...
            {selectedPort.channelGroup != null && (
              <DetailField label="Channel-Group" value={`${selectedPort.channelGroup} (${selectedPort.channelMode ?? '—'})`} />
            )}
...
function DetailField({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div style={{ fontSize: 9, color: C.textMuted, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 2 }}>
        {label}
      </div>
