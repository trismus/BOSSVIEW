...
export function VlanConsistencyPanel({
  ...
  const issues = useMemo(() => {
    ...
    const configVlanMap = new Map<
      number,
      { name: string; deviceIds: string[]; deviceNames: string[] }
    >();
    ...
    for (const link of links) {
      ...
      if (onlyOnFrom.length > 0 || onlyOnTo.length > 0) {
        ...
        if (onlyOnFrom.length > 0) {
          details.push(
            `Only on ${fromDev.name} (${fromIface.name}): ${formatVlanList(onlyOnFrom)}`,
          );
        }
              ...
              <div>
                {cat.issues.map((issue) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 10, color: C.textMuted }}>{issue.detail}</div>
                  </div>
                ))}
              </div>
            )}
